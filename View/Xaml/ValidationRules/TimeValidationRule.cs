...
namespace Fork.View.Xaml.ValidationRules
{
    public class TimeValidationRule : ValidationRule
    {
        public override ValidationResult Validate(object value, CultureInfo cultureInfo)
        {
            ...
            {
                return new ValidationResult(false,"Not a valid time format");
            }
            if(!int.TryParse(splittedTime[0].ToString(),out hours))
            {
                return new ValidationResult(false, "Hours value is not numeric");
            }
            if(!int.TryParse(splittedTime[1].ToString(),out minutes))
            {
                return new ValidationResult(false, "Minutes value is not numeric");
            }
            if (hours < 0 || hours > 23)
            {
                return new ValidationResult(false, "Hours value has to be between 0 and 23");
            }
