...
public partial class ImportPage : Page
{
    ...
    {
        versionComboBox.SetBinding(ItemsControl.ItemsSourceProperty, new Binding { Source = VersionList });
    ...
    private async void BtnApply_Click(object sender, RoutedEventArgs e)
    {
        ...
        DirectoryInfo oldDir = new(lastPath);
        ...
        if (Directory.Exists(fromPluginsFolder))
        {
            ...
            foreach (string file in jarFiles)
            {
                ...
                bool added =
                    await LocalPluginUtils.DoVirtualLoading(pluginsPage.viewModel, targetName, file, destination);
                if (added)
                {
                    viewModel.AddToConsole(
                        new ConsoleMessage(
                            $"[LOCAL PLUGIN INSTALLATION] Loaded plugin from imported server {targetName}",
                ...
                else
                {
                    viewModel.AddToConsole(
                        new ConsoleMessage(
                            $"[LOCAL PLUGIN INSTALLATION] Failed to load plugin from imported server {targetName}",
