...
public class WebSocketHandler : IDisposable
{
    ...
    public WebSocketState DiscordWebSocketState { get; }
    ...
    private int StopServer(string[] splitted)
    {
        ...
        ConsoleWriter.Write(
            new ConsoleMessage($"Discord user {splitted[2]} stopped server remotely", ConsoleMessage.MessageLevel.WARN),
            targets[0]);
    ...
    private int StartServerAsync(string[] splitted)
    {
        ...
        ConsoleWriter.Write(
            new ConsoleMessage($"Discord user {splitted[2]} started server remotely", ConsoleMessage.MessageLevel.WARN),
            targets[0]);
        Task.Run(async () =>
        {
            List<Task> tasks = new();
    ...
    {
        List<EntityViewModel> viewModels = new(ServerManager.Instance.Entities);
        List<string> resultList = new(2 + viewModels.Capacity * 6);
