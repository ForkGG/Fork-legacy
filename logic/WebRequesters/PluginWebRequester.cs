...
public class PluginWebRequester
{
    ...
    private readonly string baseURL = "http://api.spiget.org/v2/";
    private readonly int pageSize = 10;

    private readonly string resourceFields =
        "id%2Cname%2Ctag%2Ccontributors%2Clikes%2Cfile%2CtestedVersions%2Crating%2Cauthor%2Ccategory%2CreleaseDate%2Cdownloads%2Cicon%2Cpremium%2CupdateDate";

    public List<Plugin> RequestResourceList(out bool fullyLoaded, int page = 1,
        PluginEnums.Sorting sort = PluginEnums.Sorting.RATING)
    {
        ...
        if (json == null)
        {
            ...
            catch (WebException e)
            {
                ...
                Console.WriteLine(
                    "Could not receive Spigot Plugins. (either spiget.org is down or your Internet connection is not working)\nRequest URL: " +
                    url);
                fullyLoaded = true;
        ...
        List<Plugin> plugins = JsonConvert.DeserializeObject<List<Plugin>>(json);
    ...
    public List<Plugin> RequestResourceList(out bool fullyLoaded, PluginCategory category, int page = 1,
        PluginEnums.Sorting sort = PluginEnums.Sorting.RATING)
    {
        ...
        foreach (Plugin plugin in toRemove) plugins.Remove(plugin);
    ...
    public List<Plugin> RequestResourceList(out bool fullyLoaded, string searchQuery, PluginCategory category = null,
        int page = 1, PluginEnums.Sorting sort = PluginEnums.Sorting.RATING)
    {
    ...
    private Author RequestAuthor(int authorId)
    {
        ...
        if (json == null)
        {
            ...
            catch (WebException e)
            {
                ErrorLogger.Append(e);
                Console.WriteLine(
                    "Could not receive Spigot Plugin Author. (either spiget.org is down or your Internet connection is not working)\nRequest URL: " +
                    url);
    ...
    public Plugin RequestPlugin(int pluginId)
    {
        ...
        if (json == null)
        {
            ...
            catch (WebException e)
            {
                ErrorLogger.Append(e);
                Console.WriteLine(
                    "Could not receive Spigot Plugin Details. (either spiget.org is down or your Internet connection is not working)\nRequest URL: " +
                    url);
                return null;
            }
        }
    ...
    public long RequestLatestVersion(int pluginId)
    {
        ...
        if (json == null)
        {
            ...
            catch (WebException e)
            {
                ErrorLogger.Append(e);
                Console.WriteLine(
                    "Could not receive latest Spigot Plugin Version. (either spiget.org is down or your Internet connection is not working)\nRequest URL: " +
                    url);
                return 0;
            }
        }
    ...
    private string BuildResourceCategoryURL(int pageSize, PluginCategory category, int page = 1,
        PluginEnums.Sorting sort = PluginEnums.Sorting.RATING)
    {
    ...
    private string BuildResourceSearchURL(int pageSize, string searchQuery, int page = 1,
        PluginEnums.Sorting sort = PluginEnums.Sorting.RATING)
    {
