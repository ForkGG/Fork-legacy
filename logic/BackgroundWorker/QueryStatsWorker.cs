...
public class QueryStatsWorker : IDisposable
{
    ...
    private readonly Regex playerJoinFabric =
        new(@"\[([0-9]+:*)+\] \[Server thread\/INFO\]: ([0-9a-zA-Z_]+)\[.*\] logged in with entity id [0-9]+ at .*$");

    private readonly Regex playerJoinPaper = new(@"\[([0-9]+:*)+ INFO\]: ([0-9a-zA-Z_]+) joined the game$");

    private Regex playerJoinPaper2 =
        new(@"\[([0-9]+:*)+ INFO\]: ([A-Za-z0-9_]+)\[.*\] logged in with entity id [0-9]+ at \(.*\)$");

    private readonly Regex playerJoinPurpur = new(@"\[([0-9]+:*)+ INFO\]: ([0-9a-zA-Z_]+) joined the game$");

    private readonly Regex playerJoinSpigot =
        new(@"\[([0-9]+:*)+\] \[Server thread\/INFO\]: ([0-9a-zA-Z_]+)\[.*\] logged in with entity id [0-9]+ at .*$");

    private readonly Regex playerLeave = new(@"\[([0-9]+:*)+\] \[Server thread/INFO\]: ([0-9a-zA-Z_]+) left the game$");

    private readonly Regex playerLeavePaper = new(@"\[([0-9]+:*)+ INFO\]: ([0-9a-zA-Z_]+) left the game$");

    private readonly Regex playerLeavePurpur = new(@"\[([0-9]+:*)+ INFO\]: ([0-9a-zA-Z_]+) left the game$");
        ...
        if (joinMatch.Success)
        {
            ...
            try
            {
                ...
                if (!found)
                {
                    ...
                    ServerPlayer player = new(p, viewModel, viewModel.OPList.Contains(p), true);
