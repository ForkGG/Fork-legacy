...
public class Player : IEquatable<Player>
{
    ...
    private async Task RetrieveNameAndHead()
    {
        ...
        Uri uri = new Uri("https://sessionserver.mojang.com/session/minecraft/profile/" + Uid);
    ...
    {
        byte[] profileJson = Convert.FromBase64String(base64String);
        ...
        {
            HttpClient client = new HttpClient();
            Uri uri = new Uri(profile.textures.SKIN.url);
            HttpResponseMessage response = await client.GetAsync(uri, HttpCompletionOption.ResponseHeadersRead);
            while (response.StatusCode == (HttpStatusCode)429)
            {
                await Task.Delay(5000);
