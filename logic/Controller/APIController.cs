...
namespace Fork.Logic.Controller
{
    public class APIController
    {
        ...
        {
            if (IsAPIAvailable())
                return RetrieveResponseBody(RequestResponseWithCache(apiBaseURL + "ip", null, TimeSpan.FromHours(24)))
                    .Trim();
            ...
            return RetrieveResponseBody(RequestResponseWithCache("http://icanhazip.com", null, TimeSpan.FromHours(24)))
                .Trim();
        }
            ...
            TimeSpan? maxCacheAge = null)
        {
            ...
            if (maxCacheAge != null) CacheResponse(HashRequest(requestUrl, headers), response, maxCacheAge.Value);
            return response;
        }
        ...
        private void CacheResponse(string requestHash, HttpWebResponse response, TimeSpan maxCacheAge)
        {
            ...
            if (ResponseCache.ContainsKey(requestHash)) return;
        ...
        private HttpWebResponse ResponseFromCache(string requestUrl, IDictionary<string, string> headers = null)
        {
            ...
            if (ResponseCache.ContainsKey(requestHash)) return ResponseCache[requestHash].WebResponse;
            return null;
        }
        ...
        private string HashRequest(string requestUrl, IDictionary<string, string> headers = null)
        {
            ...
            foreach (byte b in hashArray) result.Append(b.ToString("X2"));
        ...
        private string HeaderDictToString(IDictionary<string, string> headers)
        {
            ...
            foreach (KeyValuePair<string, string> header in headers) headerStrings.Add(header.Key + header.Value);
            headerStrings.Sort();
