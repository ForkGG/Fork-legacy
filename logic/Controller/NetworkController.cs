...
public class NetworkController
{
        ...
        List<string> usedServerNames)
    {
        ...
        Network network = new(networkName, networkType, javaSettings, version);
        NetworkViewModel viewModel = new(network);
    ...
    public async Task<bool> StartNetworkAsync(NetworkViewModel viewModel, bool startServers)
    {
        ...
        DirectoryInfo directoryInfo = new(Path.Combine(App.ServerPath, viewModel.Network.Name));
        ...
        if (javaVersion.VersionComputed < 16)
        {
            ...
            {
                ConsoleWriter.Write(
                    "ERROR: The Java installation selected for this network is outdated. Please update Java to version 16 or higher.",
                    viewModel);
                return false;
            }

            ConsoleWriter.Write(
                "WARN: The Java installation selected for this network is outdated. Please update Java to version 16 or higher.",
                viewModel);
        }
        ...
        ConsoleReader consoleReader = new(process.StandardInput);
    ...
    public async Task<bool> DeleteNetworkAsync(NetworkViewModel networkViewModel)
    {
        try
        {
            ...
            DirectoryInfo serverDirectory = new(Path.Combine(App.ServerPath, networkViewModel.Name));
    ...
    public bool CloneNetwork(NetworkViewModel viewModel, List<string> usedEntityNames)
    {
        ...
        try
        {
            ...
            NetworkViewModel newNetworkViewModel = new(newNetwork);
