...
public class ServerViewModel : EntityViewModel
{
                ...
                restartTimer.Elapsed += (sender, args) =>
                {
                    ...
                    if (timeSpan.Hours < 0 || timeSpan.Minutes < 0 || timeSpan.Seconds < 0)
                    {
                        timeSpan = TimeSpan.FromMilliseconds(
                            ServerAutomationManager.Instance.CalculateTime(automationTime));
                    ...
                    if (timeSpan.Hours == 0)
                    {
                        ...
                        {
                            if (timeSpan.Minutes == 30 || timeSpan.Minutes == 1 ||
                                (timeSpan.Minutes <= 15 && timeSpan.Minutes % 5 == 0))
                            {
    ...
    public void InitializeWorldsList()
    {
        ...
        foreach (DirectoryInfo directory in serverDir.EnumerateDirectories())
        {
            ...
            if (worldVal.IsValid)
            {
                World world = new(worldVal.Name, this, directory);
