...
using Group = Fork.Logic.Model.ProxyModels.Group;
...
public class NetworkViewModel : EntityViewModel
{
    public NetworkViewModel(Network network) : base(network)
    {
        ...
        Application.Current.Dispatcher.Invoke(
            new Action(() => PluginsPage = new PluginsPage(new PluginViewModel(this))));
    ...
    public void AddServer(ServerViewModel serverViewModel, int index)
    {
        ...
        NetworkForkServer networkForkServer = new(serverViewModel, server, serverViewModel.Server.Name);
        ...
        if (!contains)
        {
            ...
            long lastlyAdded = 0;
            bool rename = false;
            Collection<string> toRemove = new();
            foreach (string storedName in Network.Config.servers.Keys)
            {
                Server net = Network.Config.servers[storedName];
                if (net.address == networkForkServer.Address)
                {
                    if (net.ForkServer)
                    {
                        AddToConsole(new ConsoleMessage(
                            $"Refused to add server {networkForkServer.Name} because it's already on the network",
                ...
                Match match = regex.Match(storedName);
                if (match.Success)
                {
            ...
            if (toRemove.Count > 0)
            {
                foreach (string net in toRemove)
                    try
                    {
                        ...
                        AddToConsole(new ConsoleMessage(
                            $"Removing non-fork server {net} because a Fork server with the same information has been added",
    ...
    {
        NetworkExternalServer networkExternalServer = new(server, name);
    ...
    private void LoadServersFromConfig(BungeeSettings settings)
    {
        ...
        List<NetworkServer> servers = new();
        ...
        foreach (KeyValuePair<string, Server> settingsServer in settings.servers)
        {
            ...
            if (settingsServer.Value.ForkServer)
            {
                ...
                {
                    NetworkForkServer server = new(serverViewModel, settingsServer.Value, name);
                    servers.Add(server);
                }
                else
                {
            ...
            {
                NetworkExternalServer server = new(settingsServer.Value, name);
                servers.Add(server);
            }
        }
    ...
    private void LoadPermissionsFromConfig(BungeeSettings settings)
    {
        ...
        {
            Permission p = new(keyValuePair.Key, new ObservableCollection<string>(keyValuePair.Value));
            permissions.Add(p);
        }
    ...
    private void LoadGroupsFromConfig(BungeeSettings settings)
    {
        ...
        {
            Group g = new(keyValuePair.Key, new ObservableCollection<string>(keyValuePair.Value));
            groups.Add(g);
        }
